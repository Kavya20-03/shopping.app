...
@app.route('/remove_item/<int:cart_id>', methods=['GET'])
def remove_item(cart_id):
    cart_item = Cart.query.get_or_404(cart_id)
    db.session.delete(cart_item)  # Delete the cart item from the database
    db.session.commit()  # Commit the changes to the database

    flash('Item removed from your cart', 'success')
    return redirect(url_for('view_cart'))  # Redirect to the cart view page
